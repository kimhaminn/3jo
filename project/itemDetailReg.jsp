...
<%
    ...
    try {
        ...
        ps = conn.prepareStatement(
            ...
            "ba.transport_cost, ba.delay_risk, ba.defect_rate, " +
            "v.vendor_name, v.tier, v.address, u.user_name, u.biz_no, u.corp_no, u.tech_cert, " +
            "sr.reasons, sr.reason_note, " +
            "bc.cert_kc, bc.cert_self, bc.cert_mlit, bc.cert_iso9001, bc.cert_iatf, bc.cert_oem " +
            ...
            "LEFT JOIN bid_selection_reasons sr ON sr.app_id = ba.app_id " +
            "LEFT JOIN bid_certifications bc ON bc.app_id = ba.app_id " +
        ...
        while (rs.next()) {
            appList.add(new String[]{
                ...
                rs.getInt("transport_cost") > 0
                    ...
                    : "-",
                rs.getString("reasons")           != null ? rs.getString("reasons")          : "-",// [18]
                rs.getString("reason_note")       != null ? rs.getString("reason_note")      : "-",// [19]
                rs.getString("cert_kc")           != null ? rs.getString("cert_kc")          : "-",// [20]
                rs.getString("cert_self")         != null ? rs.getString("cert_self")        : "-",// [21]
                rs.getString("cert_mlit")         != null ? rs.getString("cert_mlit")        : "-",// [22]
                rs.getString("cert_iso9001")      != null ? rs.getString("cert_iso9001")     : "-",// [23]
                rs.getString("cert_iatf")         != null ? rs.getString("cert_iatf")        : "-",// [24]
                rs.getString("cert_oem")          != null ? rs.getString("cert_oem")         : "-",// [25]
                rs.getString("delay_risk")        != null ? rs.getString("delay_risk")       : "-",// [26]
                rs.getString("defect_rate")       != null ? rs.getString("defect_rate")      : "-" // [27]
            });
        }
    ...
    <div class="main">
      ...
      <div class="section-card">
        <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:14px;">
          ...
          <% if (isScm) { %>
          <button class="btn-link btn-detail" style="padding:7px 16px;font-size:12px;"
            onclick="location.href='supplyResult.jsp?bidId=<%= bidId %>'">
            <i class="bi bi-check-circle-fill" style="margin-right:5px;"></i>공급업체선정
          </button>
          <% } %>
        </div>
...
for (int i = 0; i < appList.size(); i++) {
    ...
    String[] keys = {"appId","vendorName","quotePrice","applyDt","status",
                     ...
                     "reasons","reasonNote",
                     "certKc","certSelf","certMlit","certIso9001","certIatf","certOem",
                     "delayRisk","defectRate"};
  ...
  if (canSeeAll) {
    html +=
      ...
      sec('2-2. 가격 및 위험 정보', [
        ...
        pi('납기지연 위험요소', a.delayRisk !== '-' ? (Math.round(parseFloat(a.delayRisk) * 100)) + '%' : '-'),
        (function(){
          if (a.defectRate === '-') return pi('불량률', '-');
          var v = parseFloat(a.defectRate);
          var pct = (v * 100).toFixed(1) + '%';
          var badge = v > 0.01
            ? '<span style="color:#EF4444;font-weight:700;margin-left:6px;">⚠ 불량률 높음</span>'
            : '<span style="color:#00E5A0;font-weight:700;margin-left:6px;">✓ 불량률 낮음</span>';
          return '<div class="pop-item"><div class="pop-label">불량률</div><div class="pop-value">' + esc(a.defectRate) + ' (' + pct + ')' + badge + '</div></div>';
        })(),
        pi('공급 위험 등급', a.riskGrade),
        pi('신청일', a.applyDt),
        pi('상태', stLabel),
        a.reasons !== '-' ? piF('선정 사유', a.reasons) : '',
        a.reasonNote !== '-' ? piF('선정 사유 상세', a.reasonNote) : ''
      ]) +
      (function() {
        var certItems = [];
        if (a.certKc     !== '-') certItems.push(pi('KC인증',          a.certKc));
        if (a.certSelf   !== '-') certItems.push(pi('자가인증',         a.certSelf));
        if (a.certMlit   !== '-') certItems.push(pi('국토교통부 인증',   a.certMlit));
        if (a.certIso9001!== '-') certItems.push(pi('ISO9001',         a.certIso9001));
        if (a.certIatf   !== '-') certItems.push(pi('IATF 16949',      a.certIatf));
        if (a.certOem    !== '-') certItems.push(pi('OEM',             a.certOem));
        return certItems.length > 0
          ? sec('2-3. 인증 현황', certItems)
          : '';
      })();
  } else {
    html +=
